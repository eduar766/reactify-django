...
class PostCreate extends Component {
  ...
  handleInputChange(event){
    ...
    if(key === 'title'){
      if(value.length > 120){
        alert("Bajale dos, ese titulo es muy grande")
      }
    }
    this.setState({
      [key]: value
    ...
    return(
      <form onSubmit={this.handleSubmit}>
        ...
        <div className='form-group'>
          ...
          <input type='date' id='publish' name='publish' className='form-control' placeholder='Blog post title' onChange={this.handleInputChange} required='required' />
        </div>
